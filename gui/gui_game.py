...
class GUIGame(GUI):
    ...
    def start_game(self):
        ...
        while run:
            ...
            underscore = self.h1.render(' '.join(self.game.aesthetic_underscore), True, self.color_white)
    ...
    def show_word(self):
        u2 = ['_' if letter.isalpha() else ' ' for letter in self.game.hidden_word]
        w2 = [letter.upper() if letter != '_' else ' ' for letter in self.game.hidden_word]

        # Display word to guess, shadow missing letters:
        underscore = self.h1.render(' '.join(self.game.aesthetic_underscore), True, self.color_gray)
        underscore2 = self.h1.render(' '.join(u2), True, self.color_white)
        word = self.h1.render(' '.join(self.game.word_to_guess.upper()), True, self.color_gray)
